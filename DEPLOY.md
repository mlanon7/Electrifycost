...
```powershell
cd "D:\claude projects\Websites\Electrifycost"
```
...
- Sitemap generated by the custom `scripts/build-sitemap.cjs` post-build step (runs automatically via the `build` npm script) → `dist/sitemap.xml` (~700 URLs). NOT `@astrojs/sitemap`, which crashes against Astro 4.16 — see `.claude/lessons/01-sitemap-namespace-typo.md`.
